...
export function DistributorOrdersList() {
  const { user } = useAuth();
  ...
  // Distributor sees orders where they are involved
  const myOrders = orders.filter(o => 
    (o.requestedByRole === 'distributor' && o.requestedBy === user?.id) || 
    (o.requestedToRole === 'distributor' && o.requestedTo === user?.id)
  );
  ...
  const getEntityName = (id: string, role: string) => {
    ...
    if (role === 'partner') return partners.find(p => p.id === id)?.businessName || id;
    return id;
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Manage partner requests and company orders.</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => navigate('/distributor/orders/new-request')}>
            <Plus className="mr-2 h-4 w-4" /> Request from Company
          </Button>
          <Button onClick={() => navigate('/distributor/orders/new')}>
            <Plus className="mr-2 h-4 w-4" /> Order for Partner
          </Button>
        </div>
      </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 md:pb-0">
            ...
            <Select 
              ...
              options={[
                ...
                { label: 'From Company', value: 'company_to_distributor' },
                { label: 'To Company', value: 'distributor_to_company' },
                { label: 'To Partner', value: 'distributor_to_partner' },
                { label: 'From Partner', value: 'partner_to_distributor' },
              ]}
            />
          </div>
        </div>
