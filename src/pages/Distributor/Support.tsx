...
export function DistributorSupport() {
  const { user } = useAuth();
  ...
  const myTickets = tickets.filter(t => t.createdBy === user?.id || (t as any).recipientId === user?.id);
  ...
  const handleSubmit = () => {
    if (user) {
      createTicket({
        ...
        role: 'distributor',
        recipientId: 'company', // Escalate to company
        subject,
        description,
        category,
        priority
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500">Contact company support or manage partner requests.</p>
        </div>
