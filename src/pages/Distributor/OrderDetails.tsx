...
import { ArrowLeft, CheckCircle2, Truck, Package, PackageCheck, Settings, Play } from 'lucide-react';
...
export function DistributorOrderDetails() {
  const { id } = useParams();
  ...
  const isIncoming = order.requestedToRole === 'distributor';
  
  const getEntityName = (cId: string, role: string) => {
    if (role === 'distributor') return 'You';
    if (role === 'company') return 'Company';
    if (role === 'partner') return partners.find(p => p.id === cId)?.businessName || cId;
    return cId;
  };

  // Inventory available to the distributor to assign to a partner
  // Must be assigned to distributor, NOT assigned to partner, and active/installed/available conceptually.
  const myInventory = machines.filter(m => m.distributorId === user?.id && m.partnerId === null && ['available', 'active', 'installed'].includes(m.status));
  const availableInventoryCount = myInventory.length;

  const handleApprove = () => {
    if (availableInventoryCount < order.quantity) {
      alert(`Insufficient available inventory. Only ${availableInventoryCount} machines are currently available in your pool.`);
      return;
    }
    updateOrderStatus(order.id, 'approved', 'Distributor', 'Order approved by Distributor.');
  };

  const handleReserve = () => {
    updateOrderStatus(order.id, 'inventory_reserved', 'Distributor', 'Inventory reserved.');
  };

  const handleReject = () => {
    updateOrderStatus(order.id, 'rejected', 'Distributor', 'Order rejected.');
  };
  ...
  const handleAssignMachinesSubmit = () => {
    ...
    assignMachinesToOrder(order.id, selectedMachineIds, 'Distributor');
    ...
    selectedMachineIds.forEach(mId => {
      if (partId) {
        assignMachine(mId, user?.id || '', partId); // assign to partner
      }
    });
  ...
  const handleAdvanceState = (nextState: OrderStatus, actionText: string) => {
    updateOrderStatus(order.id, nextState, 'Distributor', `Marked as ${actionText}.`);
    ...
    if (nextState === 'dispatched') {
      ...
      order.assignedMachineIds.forEach(mId => updateMachineStatus(mId, 'installed'));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="sm" onClick={() => navigate('/distributor/orders')}>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              {/* If Partner requested from Distributor */}
              {order.status === 'requested' && isIncoming && order.requestedByRole === 'partner' && (
                <>
                  <div className="mb-4 p-3 bg-slate-50 border border-slate-200 rounded-md">
                    <p className="text-xs text-slate-500">Your Available Inventory</p>
              ...
              {/* Dist/Partner fulfillment flow */}
              {(order.status === 'approved' || (order.status === 'requested' && !isIncoming && order.requestedToRole === 'partner')) && (
              ...
              {/* If Distributor requested from Company (Company fulfills) */}
              {order.requestedByRole === 'distributor' && order.requestedToRole === 'company' && ['requested', 'approved', 'inventory_reserved', 'machines_assigned', 'dispatched'].includes(order.status) && (
                <div className="text-sm text-slate-500 p-3 bg-slate-50 rounded-md">
                  Waiting on Company to fulfill this request.
                </div>
              )}
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-slate-600">Select {order.quantity} machine(s) from your available inventory to assign to this Partner.</p>
              ...
              <tbody>
                {myInventory.map(m => (
                  <tr key={m.id} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="p-2 text-slate-500">{m.location}</td>
                  </tr>
                ))}
                {myInventory.length === 0 && (
                  <tr><td colSpan={3} className="p-4 text-center text-slate-500">No available machines found in your inventory.</td></tr>
                )}
              </tbody>
            </table>
          </div>
