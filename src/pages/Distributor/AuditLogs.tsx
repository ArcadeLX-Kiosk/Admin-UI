...
export function DistributorAuditLogs() {
  const { user } = useAuth();
  const { auditLogs: logs } = useAudit();
  const [searchTerm, setSearchTerm] = useState('');

  const myLogs = logs.filter(log => log.distributorId === user?.id || log.actorId === user?.id);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500">Track operations and data changes in your network.</p>
        </div>
      </div>
